...
import { getCashBankReport } from '@/lib/finance/reports/ledger'
...
export async function GET(request: Request) {
  ...
  try {
    const fundId = searchParams.get('fundId') ? parseInt(searchParams.get('fundId')!, 10) : undefined
    const data = await getCashBankReport(range, params, fundId)
    ...
    if (format === 'xlsx') {
      ...
      await logExportAudit(session.userId, 'cash-bank', metadata, items.length)

      return new NextResponse(buffer as any, {
        headers: {
          'Content-Disposition': `attachment; filename="laporan-cash-bank-${new Date().toISOString().split('T')[0]}.xlsx"`,
