...
import { getDisbursementReport } from '@/lib/finance/reports/disbursements'
...
export async function GET(request: Request) {
  ...
  try {
    const data = await getDisbursementReport(range, params)
    ...
    if (format === 'xlsx') {
      ...
      await logExportAudit(session.userId, 'disbursements', metadata, items.length)

      return new NextResponse(buffer as any, {
        headers: {
          'Content-Disposition': `attachment; filename="laporan-disbursements-${new Date().toISOString().split('T')[0]}.xlsx"`,
