...
import { getAcademicCollectionReport } from '@/lib/finance/reports/academic'
...
export async function GET(request: Request) {
  ...
  try {
    const data = await getAcademicCollectionReport(range, params)
    ...
    if (format === 'xlsx') {
      ...
      const metadata = {
        title: 'academic-collections Report',
      ...
      await logExportAudit(session.userId, 'academic-collections', metadata, items.length)

      return new NextResponse(buffer as any, {
        headers: {
          'Content-Disposition': `attachment; filename="laporan-academic-collections-${new Date().toISOString().split('T')[0]}.xlsx"`,
