...
import { getAcademicReconciliationReport } from '@/lib/finance/reports/academic'
...
export async function GET(request: Request) {
  ...
  try {
    const data = await getAcademicReconciliationReport()
    ...
    if (format === 'xlsx') {
      ...
      const metadata = {
        title: 'academic-reconciliation Report',
      ...
      await logExportAudit(session.userId, 'academic-reconciliation', metadata, items.length)

      return new NextResponse(buffer as any, {
        headers: {
          'Content-Disposition': `attachment; filename="laporan-academic-reconciliation-${new Date().toISOString().split('T')[0]}.xlsx"`,
