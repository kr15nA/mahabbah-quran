...
export async function GET(request: Request) {
  ...
  try {
    ...
    const data = await getReceivableAgingReport(asOfDate)
    ...
    if (format === 'xlsx') {
      ...
      await logExportAudit(session.userId, 'receivable-aging', metadata, items.length)

      return new NextResponse(buffer as any, {
        headers: {
          'Content-Disposition': `attachment; filename="laporan-receivable-aging-${new Date().toISOString().split('T')[0]}.xlsx"`,
