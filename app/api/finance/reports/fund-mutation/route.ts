...
import { getFundMutationReport } from '@/lib/finance/reports/funds'
...
export async function GET(request: Request) {
  ...
  try {
    const data = await getFundMutationReport(range)
    ...
    if (format === 'xlsx') {
      ...
      await logExportAudit(session.userId, 'fund-mutation', metadata, items.length)

      return new NextResponse(buffer as any, {
        headers: {
          'Content-Disposition': `attachment; filename="laporan-fund-mutation-${new Date().toISOString().split('T')[0]}.xlsx"`,
