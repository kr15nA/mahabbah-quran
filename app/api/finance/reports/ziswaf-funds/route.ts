...
import { getZiswafFundReport } from '@/lib/finance/reports/ziswaf'
...
export async function GET(request: Request) {
  ...
  try {
    const data = await getZiswafFundReport(range)
    ...
    if (format === 'xlsx') {
      ...
      await logExportAudit(session.userId, 'ziswaf-funds', metadata, items.length)

      return new NextResponse(buffer as any, {
        headers: {
          'Content-Disposition': `attachment; filename="laporan-ziswaf-funds-${new Date().toISOString().split('T')[0]}.xlsx"`,
