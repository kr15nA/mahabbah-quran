...
import { getZiswafReceiptReport } from '@/lib/finance/reports/ziswaf'
...
export async function GET(request: Request) {
  ...
  try {
    const data = await getZiswafReceiptReport(range, params, rawFilters)
    ...
    if (format === 'xlsx') {
      ...
      await logExportAudit(session.userId, 'ziswaf-receipts', metadata, items.length)

      return new NextResponse(buffer as any, {
        headers: {
          'Content-Disposition': `attachment; filename="laporan-ziswaf-receipts-${new Date().toISOString().split('T')[0]}.xlsx"`,
