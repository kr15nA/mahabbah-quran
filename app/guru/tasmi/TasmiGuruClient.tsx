...
import { createGuruTasmiAction, updateGuruTasmiAction, deleteGuruTasmiAction } from './actions'
...
interface TasmiGuruClientProps {
...
export function TasmiGuruClient({
  ...
  const handleSaveTasmi = async (formData: TasmiFormData) => {
    ...
    try {
      if (editingRow) {
        const res = await updateGuruTasmiAction(editingRow.id, {
        ...
        if (!selectedStudentId) throw new Error('Pilih santri terlebih dahulu')
        const res = await createGuruTasmiAction({
  ...
  const handleDelete = async (row: TasmiHistoryRow) => {
    ...
    if (confirm(`Hapus data Tasmi ini?\n\nSantri: ${row.student_name}\nTanggal: ${new Date(row.session_date).toLocaleDateString('id-ID')}\nTarget: ${targetTxt}`)) {
      startTransition(async () => {
        try {
          const res = await deleteGuruTasmiAction(row.id)
          if (!res.success) throw new Error(res.error)
          router.refresh()
        } catch (err: any) {
    ...
    <div className="space-y-4 pb-10">
      <div className="bg-white p-4 rounded-2xl border border-gray-200 shadow-sm flex flex-col md:flex-row justify-between md:items-center gap-4">
        <div>
          ...
          <p className="text-xs text-gray-500">Rekapitulasi dan ujian Tasmi santri Anda</p>
        </div>
